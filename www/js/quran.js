...
export default async () => {
    try {
        ...
        if (window.location.pathname.endsWith('/quran.html')) {
            ...
            let quran_back = document.getElementById('quran_back');

            quran_back.addEventListener("click", e => {
...
function handleReaderClick(item, mp3quran) {
    try {
        ...
        quranReaderSurahHeaderBack.addEventListener("click", e => {
            quranReaderSurah.style.display = "none";
            quranReaderSurahHeader.style.display = "none";
            quranMp3H3.style.display = "block";
            quranReader.style.display = "grid";
            searchReader.style.display = "block";
            
            // إيقاف الصوت عند العودة
            if (currentAudio) {
                currentAudio.pause();
                currentAudio.src = "";
                if (currentPlayButton) {
                    currentPlayButton.className = "ph ph-play play_btn";
                }
                isAudioPlaying = false;
            }
            
            // مسح قائمة السور لتوفير الذاكرة
            quranReaderSurah.innerHTML = "";
            window.scrollTo(0, 0);
        });
        ...
        for (let iterator of item?.audio) {
            let li = document.createElement("li");
            let surahInfo = document.createElement("div");
            ...
            surahInfo.className = "surah_info";
            li.appendChild(surahInfo);

            surahNumber.className = "surah_number";
            surahNumber.innerText = iterator?.id;
            surahInfo.appendChild(surahNumber);

            h3.innerText = iterator?.name;
            surahInfo.appendChild(h3);

            quranReaderSurahImg.className = "surah_actions";
            li.appendChild(quranReaderSurahImg);

            playButton.id = `quran_reader_surah_img_play_${iterator?.id}`;
            playButton.className = "ph ph-play play_btn";
            quranReaderSurahImg.appendChild(playButton);

            download.id = `quran_reader_surah_img_download_${iterator?.id}`;
            download.className = "ph ph-download play_btn"; // Using same style for consistency
            ...
            playButton.addEventListener("click", async () => {
                ...
                if (!isAudioPlaying) {
                    playButton.className = "ph ph-circle-notch ph-spin play_btn";
                    isMediaPlayStorage(false);
                    isPausedStorage(true);
...
function playAudio(audio, playButton) {
    audio.play().then(() => {
        ...
        isAudioPlaying = true;
        playButton.className = "ph ph-stop play_btn playing";
    }).catch((error) => {
